...
class UserTextInterface
  ...
  def take_place
    ...
    if vagen_number > 0 && vagen_number < train_obj.vagens.length
  ...
  def show_trains_vagen
    ...
    train_obj.show_vagens unless train_obj.nil?
  end

  def get_train_obj(train_id)
    train_obj = nil
    Train.all.each do |t|
      if t.vagen_id == train_id
        train_obj = t
        break
      end
    end
  ...
  def validate_train!(train_type, vagen_qty)
    ...
    raise 'Количество вагонов указано не верно!' if vagen_qty.nil? || vagen_qty < 0
  end
end
